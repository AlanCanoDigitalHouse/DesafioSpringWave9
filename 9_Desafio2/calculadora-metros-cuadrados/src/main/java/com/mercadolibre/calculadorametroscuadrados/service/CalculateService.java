...
public class CalculateService {
    ...
    public HouseResponseDTO calculate(HouseDTO house) {
        priceRepository.findPriceLocation(house.getAddress().getLocation());
        HouseResponseDTO response = new HouseResponseDTO();
        calculateRoomSquareFeet(house, response);
        response.setPrice(calculatePrice(response.getSquareFeet(), house.getAddress().getPrice()));
        return response;
    }
    ...
    private void calculateRoomSquareFeet(HouseDTO house, HouseResponseDTO response) {
        ...
        Integer maxRoom = 0;
        if (Objects.nonNull(house) && Objects.nonNull(house.getRooms())) {
