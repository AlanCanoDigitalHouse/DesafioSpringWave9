...
class ControllerTests {
  ...
  // Se carga una lista de distritos con un unico nombre: "demo"
  @BeforeEach
  public void initDistricts() throws Exception {
    this.mockMvc.perform(
            post("/loadDistricts")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{\"districts\" : [\"demo\"]}"));
  }
  ...
  void calculateRoomsSquareFeet() throws Exception {
    ...
    this.mockMvc.perform(
        post("/calculate")
            ...
            .content(request))
        .andDo(print()).andExpect(status().isOk())
        .andExpect(jsonPath("$.enviroments[0].squareFeet").value(25))
        .andExpect(jsonPath("$.enviroments[1].squareFeet").value(9))
        .andExpect(jsonPath("$.enviroments[2].squareFeet").value(2));

  }

  @Test
  void calculateSquareFeetWithNotLoadedDistrict() throws Exception {
    String districtName = "notLoadedDistrictName";
    String request = "{\"name\": \"Oficina\", \"district\":" +
            getDistrict(districtName, 800 ) + "," +
            "\"enviroments\": [" +
            getRoom("Cocina", 3, 3)  +
            "]}";
    this.mockMvc.perform(
            ...
            .andExpect(jsonPath("$.message").value("District \"" + districtName +"\" does not exist or was not loaded"));
  }
