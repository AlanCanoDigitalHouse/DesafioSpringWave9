...
public class ProductController {
    ...
    public ResponseEntity<PublicationsResponse> getPublicationByFollowed(@PathVariable int userId, @RequestParam(required = false) String order) throws ParseException, UserBadRequest {
        return new ResponseEntity<PublicationsResponse>(service.getPublicationByFollowed(userId,order), HttpStatus.OK);
    }

    @PostMapping("/newpromopost")
    public ResponseEntity<Publication> postPromotion(@Valid @RequestBody Publication promotion) throws UserBadRequest {
        return new ResponseEntity<Publication>(service.postPromotion(promotion), HttpStatus.OK);
    }

    @GetMapping("/{userId}/countPromo/")
    public ResponseEntity<PromotionCountResponse> getCountProm(@PathVariable int userId) throws UserBadRequest {
        return new ResponseEntity<PromotionCountResponse>(service.getCountProm(userId), HttpStatus.OK);
    }

    @GetMapping("/{userId}/list/")
    public ResponseEntity<PromProductsResponse> getProductsByProm(@PathVariable int userId, @RequestParam(required = false) String order) throws UserBadRequest {
