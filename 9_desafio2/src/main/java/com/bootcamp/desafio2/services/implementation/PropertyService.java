    ...
    public ResponseDto calculatePrice(PropertyDto property) throws ErrorMessage, IOException, DistrictNotFoundException {
        ...
        try {
            for (EnvironmentDto Environment: property.getEnvironments()) {
                Double area = Environment.getEnvironmentWidth() * Environment.getEnvironmentLength();
                Environment.setSquareMeters(area);
                totalArea = totalArea + area;
                if (area > maxArea) {
                    aux = Environment.getEnvironmentName();
                    maxArea = area;
                }
            }
            return new ResponseDto(totalArea, totalArea *property.getDistrict().getDistrictPrice(),
